  ...
  methods: {
      ...
      listUser(this.addDateRange(this.queryParams, this.dateRange)).then(
        response => {
          ...
          for (let index = 0; index < this.userList.length; index++) {
            this.userList[index].phonenumber = Base64.decode(
              this.userList[index].phonenumber
            );
          }
    ...
    handleUpdate(row) {
      ...
      getUser(userId).then(response => {
        ...
        // base64
        const Base64 = require("js-base64").Base64;
        this.form.email = Base64.decode(this.form.email);
        this.form.phonenumber = Base64.decode(this.form.phonenumber);
    ...
    handleResetPwd(row) {
      this.$prompt('请输入"' + row.userName + '"的新密码', "提示", {
        ...
        .then(({ value }) => {
          resetUserPwd(row.userId, this.decrypt(value)).then(response => {
    ...
    submitForm: function() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          // 对象直接赋值修改新对象，旧对象会被影响
          let pushForm = JSON.parse(JSON.stringify(this.form));
          const Base64 = require("js-base64").Base64;
          pushForm.phonenumber = Base64.encode(pushForm.phonenumber);
          pushForm.email = Base64.encode(pushForm.email);
          console.log(this.form);
          if (this.form.userId != undefined) {
            updateUser(pushForm).then(response => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            decrypt(pushForm.password).then(res => {
              pushForm.password=res;
              addUser(pushForm).then(response => {
